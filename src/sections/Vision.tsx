...
const AUDIENCE = [
  { label: "The serial quitter.", dim: false },
  { label: "The desk-bound.", dim: true },
  { label: "The gamer.", dim: false },
  { label: "The grandparent.", dim: true },
]
...
export function Vision() {
  return (
    <section
      id="vision"
      className="relative flex min-h-svh w-full snap-start flex-col justify-center overflow-hidden bg-volt px-[clamp(20px,5vw,80px)] py-[clamp(56px,8vh,110px)] text-ink"
    >
      <div className="mx-auto w-full max-w-[1180px]">
        <motion.div
          ...
          viewport={VIEWPORT}
        >
          <Eyebrow code="S03" label="Vision" />
        </motion.div>

        <motion.h2
          className="stretch-expanded mt-[clamp(12px,1.8vh,22px)] text-[clamp(36px,5.8vw,84px)] font-black uppercase leading-[0.95] tracking-[-0.01em]"
        ...
        >
          <p className="mt-[clamp(18px,3vh,34px)] flex flex-wrap items-center gap-x-[clamp(14px,1.6vw,24px)] gap-y-1">
            {AUDIENCE.map(({ label, dim }) => (
              <span
                key={label}
                className={`text-[clamp(16px,1.5vw,23px)] font-semibold tracking-[-0.01em] ${dim ? "text-ink/45" : ""}`}
              >
                {label}
              </span>
            ))}
          </p>
          <p className="mt-[clamp(10px,1.4vh,16px)] max-w-3xl font-serif text-[clamp(19px,1.7vw,27px)] italic leading-snug [text-wrap:pretty]">
            Anyone who knows exercise works &mdash; but can&rsquo;t stand doing
            it.
          </p>
        </motion.div>

        <motion.div
          className="mt-[clamp(20px,3.4vh,38px)] grid items-start gap-[clamp(24px,5vw,72px)] border-t border-ink/20 pt-[clamp(16px,2.4vh,26px)] md:grid-cols-2"
        ...
        >
          <motion.div variants={rise}>
            <span className="stretch-expanded block text-[clamp(46px,6vw,86px)] font-black leading-[0.95] tracking-[-0.02em]">
              1 in 3
            </span>
